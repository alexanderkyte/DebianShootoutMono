...
namespace BenchmarkDebianShootout
{
	...
	public class Fannkuchredux
	{
	  ...
	  [Params(7)]
		...
		public void Run ()
		{
			fact = new int[maxDepth+1];
			fact[0] = 1;
			var factn = 1;
			for (int i=1; i<fact.Length; i++) { fact[i] = factn *= i; }

			taskCount = maxDepth >9 ? factn/(7*6*5*4*3*2) : Environment.ProcessorCount;
			...
			{
				(threads[i] = new Thread(() => run(maxDepth, taskSize))).Start();
			}
			run(maxDepth, taskSize);
